        ...
        try:
            ...
            invoice_data = get_document_info(
                ...
                tax_rates=tax_rates, 
                    ...
                    # Add the API access token from the current Sales E Invoice document
            invoice_data["api_access_token"] = self.api_access_token
            ...
            frappe.log_error(f"Failed to send e-invoice: {str(e)}", "E-Invoice Submission Error")
...
def fetch_sales_invoice_details(sales_invoice):
    # try:
        ...
        try:
            ...
            frappe.throw(f"Error: Unable to fetch details for Sales Invoice {sales_invoice}. Please ensure that the tax category and TAX charges are correctly filled out before fetching the invoice details. Detailed error: {str(e)}")
        ...
        if tax_table:
            for row in tax_table.find_all('tr')[1:]:
                ...
                frappe.throw(f"No tax table found for Sales Invoice {sales_invoice}")
    ...
    # except Exception as e:
    #     frappe.throw(f"An error occurred while fetching Sales Invoice details: {str(e)}")
...
def get_document_info(
    ...
    print("from send_invoice invoice_data",invoice_data)
    return invoice_data

def send_einvoice(invoice_data):
    try:
        # The token is directly fetched from the 'api_access_token' field in the sales_e_invoice
        api_access_token = invoice_data["api_access_token"]
        ...
        if response.status_code in [200, 202]:
            ...
            # If there are accepted documents
            if accepted_documents:
                ...
                frappe.msgprint(f"Invoice submitted successfully.")
                return {
            ...
            # If there are rejected documents
                ...
                invoice_code_number = rejected_documents[0].get('invoiceCodeNumber')
                error_details = rejected_documents[0].get('error', {})
                error_message = error_details.get('message', 'Unknown error')
                document_status = 'Invalid'

                frappe.msgprint(f"Invoice submission failed. Status: {document_status}. Reason: {error_message}")
                frappe.log_error(f"Invoice submission failed. Status: {document_status}. Reason: {error_message}", "E-Invoice Error")
                return None

            else:
                # Handle case where there are neither accepted nor rejected documents
                error_message = "No accepted or rejected documents in the response."
                print(error_message)
                frappe.log_error(error_message, title="E-Invoice Error")
                frappe.throw("No document summary returned from e-invoice service. Please check Error logs list for details.")
